...
Command the robot to rotate in place and measure how accurately the IMU
...
SETTLE_TIME = 1.0               # seconds after stop to let IMU settle
...
class ImuRotationResponse(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'Starting IMU rotation response test on topic: {IMU_TOPIC}')
    ...
    def finish_and_exit(self, status_override=None, notes_override=None):
        ...
        if status_override is not None:
            ...
            self.get_logger().info('=== IMU Rotation Response Results ===')
        ...
        append_result(
            test_name='imu_rotation_response',
            status=status,
    ...
    def loop(self):
        ...
        if self.done:
            if now - self.finish_time > 0.5:
                self.get_logger().info('Exiting imu_rotation_response')
                rclpy.shutdown()
            return
