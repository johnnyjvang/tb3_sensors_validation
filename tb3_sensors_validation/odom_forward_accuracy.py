...
Command the robot to drive forward and measure how accurately odometry
tracks the requested travel distance.

Goal:
- Command a fixed forward motion
...
class OdomForwardAccuracy(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'Starting odom forward accuracy test on topic: {ODOM_TOPIC}')
    ...
    def loop(self):
        """
        Main control loop for the forward accuracy test.
        """
        now = time.time()
        ...
        if elapsed > MAX_TEST_TIME:
            self.finish_and_exit(
                ...
                notes_override='forward accuracy test timed out'
            )
            return
